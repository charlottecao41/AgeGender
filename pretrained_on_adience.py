...
model_dict={'finetune':Gender(None,3, gender_classes=2).to(DEVICE)}
...
for gender_model in model_dict:
    ...
    model.load_state_dict(torch.load('celeba_model/best.pt'))
    set_parameter_requires_grad(model)
    model.gender_head=nn.Linear(512, 2).to(DEVICE)
    ...
    plt.plot([i for i in range((len(loss_list)))],t_loss, label = "train")
    plt.plot([i for i in range(len(loss_list))],loss_list, label = "valid")
    plt.legend()
    plt.savefig(f'graph/{model_name}loss.png')
    
    plt.figure(figsize=(10,8))
    plt.plot([i for i in range((len(loss_list)))],t_acc, label = "train")
